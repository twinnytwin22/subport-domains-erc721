...
contract subportDomains721 is ERC721URIStorage, Pausable {
    ...
    function price(string calldata name) public view returns (uint) {
        ...
        if (isBeta) {
            ...
            return 10 * 10 ** 18; //10 matic for premium domain names once beta is over
        } else if (len == 4) {
            return 5 * 10 ** 18; // 5
        } else {
            return 3 * 10 ** 18; // 5
  ...
  function setRole(string calldata name, string calldata role) public nameArgsOK(name, role){
 ...
 function getTokenJson(
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                ...
                '.', tld,
                '",'
                '"description": "your official subport handle",'
    ...
    function reserve (
       ...
       string memory finalTokenUri = string(
            abi.encodePacked("data:application/json;base64,", getTokenJson(_name, role, finalSvg))
        );
